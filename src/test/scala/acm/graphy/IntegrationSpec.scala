...
class IntegrationSpec
  ...
  "LinkedOut application" should {
    ...
    "update and consume from different nodes" in {
      val graphnode1 = ClusterSharding(testKit1.system).entityRefFor(GraphNode.EntityKey, "graphnode-1")
      val probe1 = testKit1.createTestProbe[GraphNode.Confirmation[Company]]

      val graphnode2 = ClusterSharding(testKit2.system).entityRefFor(GraphNode.EntityKey, "graphnode-2")
      val probe2 = testKit2.createTestProbe[GraphNode.Confirmation[Company]]

      val eventProbe3 = testKit3.createTestProbe[GraphNode.Event]()

      testKit3.system.eventStream ! EventStream.Subscribe(eventProbe3.ref)

      // update from node1, consume event from node3
      graphnode1 ! GraphNode.AddRelationship("foo", "bez", "buz", probe1.ref)
      probe1.expectMessageType[GraphNode.Accepted[Company]]
      eventProbe3.expectMessage(GraphNode.RelationshipAdded("graphnode-1", "foo", "bez", "buz"))

      // update from node2, consume event from node3
      graphnode2 ! GraphNode.AddRelationship("bar", "ben", "bug", probe2.ref)
      probe2.expectMessageType[GraphNode.Accepted[Company]]
      graphnode2 ! GraphNode.UpdateRelationship("bar", "bez", "buz", probe2.ref)
      probe2.expectMessageType[GraphNode.Accepted[Company]]
      eventProbe3.expectMessage(GraphNode.RelationshipAdded("graphnode-2", "bar", "ben", "bug"))
      eventProbe3.expectMessage(GraphNode.RelationshipUpdated("graphnode-2", "bar", "bez", "buz"))
    }
    ...
    "continue even processing from offset" in {
      ...
      val eventProbe4 = testKit4.createTestProbe[GraphNode.Event]()
      ...
      val graphnode3 = ClusterSharding(testKit1.system).entityRefFor(GraphNode.EntityKey, "graphnode-3")
      val probe3 = testKit1.createTestProbe[GraphNode.Confirmation[Company]]

      // update from node1, consume event from node4
      graphnode3 ! GraphNode.AddRelationship("abc", "bez", "buz", probe3.ref)
      probe3.expectMessageType[GraphNode.Accepted[Company]]
      ...
      eventProbe4.expectMessage(GraphNode.RelationshipAdded("graphnode-3", "abc", "bez", "buz"))
    }
