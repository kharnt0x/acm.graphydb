...
abstract class EventProcessorStream[Event: ClassTag](
  ...
  private def readOffset(): Future[Offset] = {
    session
      .selectOne(
        "SELECT timeUuidOffset FROM acm_graphy.offsetStore WHERE eventProcessorId = ? AND tag = ?",
        eventProcessorId,
        tag)
      .map(extractOffset)
  }
    ...
    offset match {
      case t: TimeBasedUUID =>
        session.executeWrite(
          "INSERT INTO acm_graphy.offsetStore (eventProcessorId, tag, timeUuidOffset) VALUES (?, ?, ?)",
          eventProcessorId,
          tag,
          t.value)
