...
object Main {
  ...
  def startNode(port: Int, httpPort: Int): Unit = {
    val system = ActorSystem[Nothing](Guardian(), "LinkedOut", config(port, httpPort))
  ...
  def createTables(system: ActorSystem[_]): Unit = {
    ...
    val keyspaceStmt = """
      CREATE KEYSPACE IF NOT EXISTS acm_graphy
      WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 }
      """

    val offsetTableStmt =
      """
      CREATE TABLE IF NOT EXISTS acm_graphy.offsetStore (
...
object Guardian {
  def apply(): Behavior[Nothing] = {
    Behaviors.setup[Nothing] { context =>
      ...
      val httpPort = context.system.settings.config.getInt("linkedout.http.port")

      GraphNode.init[Company](system, settings)
      GraphNode.init[Person](system, settings)
      if (Cluster(system).selfMember.hasRole("read-model")) {
        EventProcessor.init(
          system,
          settings,
          tag => new LinkedOutEventProcessorStream(system, system.executionContext, settings.id, tag))
      }

      val routes = new LinkedOutRoutes()(context.system)
      new LinkedOutServer(routes.linkedOut, httpPort, context.system).start()
